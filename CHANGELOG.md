...
- **Experimental Veilid transport** (`VEILID-001`..`015`, requires building with `--features veilid`). `tetron create --veilid` / `tetron join --veilid` (mutually exclusive with `--tor`) start a real embedded Veilid node and register it as a custom transport on the shared endpoint; the join/reconnect handshake propagates each peer's Veilid identity through the signed roster, and the dial path resolves it into an address for iroh to route through the custom transport. Live-verified via `tetron-testsuite`, across several rounds of found-and-fixed bugs:
  ...
  - **Still open (`VEILID-007`):** every individual piece (`VEILID-006` through `VEILID-015`, plus the `Safe`/`Unsafe` decision) is now confirmed live in isolation; a final end-to-end re-test of all of them together is next before marking this resolved. Not yet usable for real Veilid-carried mesh traffic.
