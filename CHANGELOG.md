...
- **Experimental Veilid transport** (`VEILID-001`..`009`, requires building with `--features veilid`). `tetron create --veilid` / `tetron join --veilid` (mutually exclusive with `--tor`) start a real embedded Veilid node and register it as a custom transport on the shared endpoint; the join/reconnect handshake propagates each peer's Veilid identity through the signed roster, and the dial path resolves it into an address for iroh to route through the custom transport. Live-verified via `tetron-testsuite`, across several rounds of found-and-fixed bugs:
  ...
  - **Still open (`VEILID-007`, not yet resolved):** even with every one of the above fixed and confirmed correct, a live two-node test still shows zero Veilid traffic — evidence points to iroh racing multiple concurrent connection objects per dial attempt and keeping only the fastest, with the Veilid-carrying attempt landing on one of the discarded ones. Two rounds of live re-testing (`VEILID-008`, `VEILID-009`) instead found and fixed real roster-propagation gaps that fully explain every symptom observed so far, with no need for the racing theory — but end-to-end delivery is still unconfirmed pending a re-test against `VEILID-009`. `tests/veilid-smoke.sh` in `tetron-testsuite` (not part of the default test run) continues to correctly fail until this is resolved. Not yet usable for real Veilid-carried mesh traffic.
