...
All notable changes to tetron are documented here. The format is based on
...
- **`--tor` transport now actually carries traffic end-to-end** (`TOR-DIAL-001`), live-verified between two genuinely separate real machines on separate networks (`tetron status --json` showing `conn_type: Tor` with sustained, growing real traffic). Twelve fixes were required across tetron core and three vendored crates (`iroh`, `iroh-tor-transport`, `torut`): the Tor custom transport's own onion-address candidate was never actually offered to the mesh dial path, so `--tor` silently connected over Direct/Relay only (`connect_to_peer_with_alpn` now injects it, mirroring Veilid's own roster-carried identity); a stale-path retry gap in vendored `iroh`; an unbounded SOCKS5 connect and a missing/insufficient publish-confirmation wait in vendored `iroh-tor-transport` (raised from one HSDir confirmation to the full ~8-HSDir quorum Tor's v3 spec requires, then further to require that quorum go quiet for a settle window before trusting it, since introduction points kept churning past first quorum); an event-name validator bug in vendored `torut` blocking that wait from ever subscribing; a side-effect-in-a-retried-path bug that built a second onion service on a port-bind retry, tearing the first down mid-flight (now built exactly once); a connect-stampede bug where every outbound packet during setup raced its own fresh SOCKS5 connect (cut connection churn from ~146/5min to single digits); a too-tight 30s connect timeout cutting off Tor's own normal multi-cycle rendezvous establishment (raised to 90s); enabling Tor's `ExtendedErrors` SOCKS5 codes (via a newly-vendored `tokio-socks`) to get specific, named failure reasons instead of generic ones; and the actual final root cause — `ADD_ONION`'s non-detached ephemeral service is torn down the instant its owning control connection closes, and nothing kept that connection alive past initial endpoint setup, so the hidden service was deleting itself moments after every daemon start regardless of any other fix (`MeshManager` now holds an explicit keepalive reference for the daemon's whole lifetime). See `spec/core.py`'s `TorDialPathWiring` (Fixes 1-12) for the complete investigation.
...
- **`tetron config set path-preference <direct|relay|tor|veilid>`** (`PATHPREF-001`) lets a user force a specific transport to actually carry application data — not merely be reported as selected — once a candidate of that type has real, confirmed activity; falls back to iroh's own RTT-based selection (unchanged from before this feature existed) whenever no preference is set or the preferred transport has no validated candidate yet, so a connection is never stranded on an unproven path. Daemon-wide (one shared iroh endpoint serves every joined network), live-reloads an already-running daemon the same way `log-level` does (LOG-004 pattern) — no restart needed. Required widening a few iroh internals from crate-private to `pub` in the vendored copy (`vendor/iroh-1.0.3/PATCH.md`, Patch 6): `PathSelector` and its supporting types were reachable only from inside iroh itself despite `Endpoint::path_selector()` being a public builder method that takes one. A first live end-to-end check found "applied immediately" wasn't actually true: iroh only ever reconsiders its selection reactively (a path becoming established/abandoned), never because a `PathSelector`'s own criteria changed — a live-reloaded preference updated correctly but had no visible effect until some unrelated event happened to fire next, possibly a long time on an already-stable connection. Fixed with a small periodic re-evaluation timer in iroh's own connection actor (Patch 7, 3s interval, cheap — a pure read of already-cached path stats, no I/O) rather than relying only on existing event-driven triggers. Re-verified live after the fix: setting the preference correctly flipped the selected transport, and unsetting it correctly reverted to the default.
- **`tetron config set relay off`** (`RELAY-001`) disables relay entirely — distinct from the existing `n0`/empty, which resets to the default n0 preset rather than removing relay as a candidate. Maps to iroh's own `RelayMode::Disabled`; `tetron config get relay` shows `off`, `tetron config unset relay` clears it back to default. Primarily useful for constructing a topology where Direct and Relay are both genuinely unreachable, to test a custom transport (Tor/Veilid) as the actual primary path rather than an unused backup — previously only possible via the workaround of pointing `relay` at a deliberately unreachable URL.
- **Experimental Veilid transport** (`VEILID-001`..`016`, requires building with `--features veilid`). `tetron create --veilid` / `tetron join --veilid` (mutually exclusive with `--tor`) start a real embedded Veilid node and register it as a custom transport on the shared endpoint; the join/reconnect handshake propagates each peer's Veilid identity through the signed roster, and the dial path resolves it into an address for iroh to route through the custom transport. Live-verified via `tetron-testsuite`, across several rounds of found-and-fixed bugs:
  - A severe crash-loop bug where a `--veilid` network could take down the *entire* daemon on every restart (identity/attachment resolution blocked the whole startup sequence, not just Veilid — confirmed over 9+ restart cycles before the fix); both now resolve fully in the background instead (`VEILID-005`).
  - A missing `VeilidAPI::attach()` call meant the embedded node never actually began attaching to the public network at all — `veilid-core`'s own `api_startup()` only constructs its context, attach is a separate explicit call. Identity now resolves in ~1-2s and full attachment in ~7s, down from multiple minutes or never (`VEILID-006`).
  - `MeshManager::veilid_node_id` is now a live-updating value (a background task republishes it into a coordinator's own roster entry every 5s once known), not a one-shot boot-time snapshot that was `None` far more often than not (`VEILID-006`).
  - A real gap in the vendored `iroh` dependency itself, found live while verifying the above: a custom transport's candidate address was silently dropped whenever a faster path (Direct, on a shared LAN) won the connection race outright — relay addresses got this "keep as backup path" treatment, custom transports did not, and even once fixed, a newly-opened backup path needs to be explicitly pinged before QUIC will send anything on it at all. Patched in `vendor/iroh-1.0.3` (`vendor/iroh-1.0.3/PATCH.md`, Patch 2) — both fixes confirmed live doing exactly what they're meant to.
  - A member's `veilid_node_id` could never be corrected in the coordinator's published roster after that member's initial admission, no matter how many times it reconnected with a fresh, correct Veilid identity — the coordinator's per-member control reader received every reconnect `MeshHello` (which does carry the member's current identity) but silently discarded it, a stale carve-out from before that message also carried this field. Combined with Veilid only actually attaching on a daemon's *second* boot, this left every member other than the coordinator itself permanently stuck with no working Veilid dial candidate (`VEILID-008`).
  - A live re-test of the fix above still showed zero Veilid path activity — debug logging pinned it to a second, distinct gap: a member's identity only reaches the coordinator when the *member's own* reconnect dial wins the race after a restart; when the *coordinator's* dial wins instead (equally likely), its one-way `MeshHello` only ever announces its own identity, so the member's identity never arrives at all. A new member-side periodic re-announce (5s cadence, mirroring the coordinator's own VEILID-006 watcher) closes this regardless of which side dialed (`VEILID-009`).
  - A live re-test of *that* fix still showed nothing arriving at the coordinator, despite the periodic announce firing correctly every 5s — traced to a structural, pre-Veilid gap: `spawn_coordinator_dial_retry` (the task that owns a connection whenever the coordinator's own dial wins a reconnect race) registered a data-plane reader but never a control-message listener at all, so nothing the member ever sent back on that connection — a `Ping`, the identity announce, anything — was ever read (`VEILID-010`).
  - A real root cause, found by reading `veilid-core`'s own source rather than more live testing: `RoutingContext::app_message(Target::NodeId, ..)` — the exact call this transport's `poll_send` makes — is unconditionally rejected with `InvalidTarget` unless `veilid-core` is built with its own `footgun-nodeid-target` Cargo feature, never enabled here. Every send has failed instantly since before this investigation began, invisibly: `poll_send` is fire-and-forget (the failure never reaches iroh, which just sees a ping that never gets a response — indistinguishable from an ordinary black hole), and the failure log itself sits at `debug!` on this crate's own tracing target, which `tetron`'s file-log filter (`LOG-003`) never promotes past `info` for a dependency crate regardless of the configured log level. Confirmed independently before the fix, not just by inspection: a working sibling project (`tailveil`, a separate Tailscale-alike built on the identical `veilid-core` 0.5.7 with the identical `Target::NodeId` send call) enables exactly this feature. Fixed by enabling it (`VEILID-011`); the `Safe`-vs-`Unsafe` routing mode itself is a separate decision, resolved below.
  - A live re-test still showed zero Veilid path activity. Temporarily promoting the vendored iroh's own trace logging for one diagnostic run (reverted before committing) showed the Veilid candidate genuinely opening as a real path — proving `VEILID-011` correct — but on a `noq::Connection` object that lost a race to a *different* connection this node's own code had independently started to the same peer within the same restart window (the member's reconnect loop, the coordinator's dial-retry, and the initial roster dial-out can all fire concurrently after a disconnect, and iroh creates a separate connection object per call rather than deduplicating them). The winning connection simply discarded the Veilid-carrying connection's paths along with it. Closed with a small per-peer "dial already in flight" claim (`MeshCtx::dial_in_flight`/`DialInFlightGuard`) applied at all four of this node's own concurrent dial call sites, so only one is ever in flight to a given peer at a time — generic connection-establishment hygiene that benefits every transport, not just Veilid (`VEILID-012`).
  - The ping-after-open fix from `VEILID-007`'s own earlier vendored-iroh patch lived in the wrong place: it pinged a newly-opened path from inside the function whose *own* open attempt almost always fails with `RemoteCidsExhausted` (the connection is milliseconds old) — the real, successful open always happens moments later via a different retry code path the ping call never covered. The path reliably opened, real `PathId` and all; nothing ever pinged it, for this entire investigation. Fixed by moving the ping into the one place a path ID is ever newly assigned, covering every caller uniformly (`VEILID-013`, `vendor/iroh-1.0.3/PATCH.md` Patch 3) — confirmed live: real bidirectional Veilid `AppMessage` traffic for the first time in this investigation.
  - That traffic then stopped and the path disappeared from `tetron status` entirely, ~90s after the last confirmed activity, with no error in between. `noq_proto`'s own docs are explicit: a backup path with no `max_idle_timeout` set expires. Relay never needed one on its backup paths (kept alive incidentally by the relay protocol's own keepalive outside QUIC); a Veilid backup path has no such side channel. Fixed with a deliberately generous override (`VEILID-014`, Patch 4) — confirmed live: a Veilid backup path now survives a full settle window with `has_activity: true`, the first time this investigation ever produced that result.
  - Separately, that surviving path reported `conn_type: "Tor"` instead of `"Veilid"` — a status-classification bug predating Veilid entirely (`classify_candidate_addr` defaulted every custom transport to Tor since Tor was the only one that existed when it was written), caught only because the path's own `remote_addr` still carried the real Veilid transport id. Fixed by checking the actual transport id before falling back to Tor (`VEILID-015`).
  - The mechanism is now confirmed working end-to-end, but only under `SafetySelection::Unsafe` (direct Veilid send, no sender-anonymizing safety route) — the intended default, `SafetySelection::Safe`, never once delivered a message across this entire investigation despite reporting local send success every time. Decided: `veilid-transport` now uses `Unsafe` deliberately. Tetron's own peers already know each other's identity directly (invite-gated mesh, not anonymous), and Veilid is a last-resort fallback ranked below Tor, which already provides the more mature anonymity property for whoever actually needs it — a working transport beats a non-functional one. Revisiting `Safe` mode later remains possible but isn't required.
  - **`VEILID-007` closed 2026-09-12**: `tests/veilid-smoke.sh` passed with every fix from `VEILID-006` through `VEILID-015` plus the `Unsafe` decision in place together for the first time — both nodes, both directions, a real Veilid path candidate with confirmed activity. Still not tried against a topology where Direct and Relay are genuinely unreachable (not merely outranked) — this smoke test only proves the mechanism works as a backup path alongside a working Direct connection.
  - A ~1-in-3 flake surfaced immediately after: re-running the same smoke test against the same commit, no code changes, sometimes produced zero Veilid path activity at all. Root cause: the transport's own `poll_send` is fire-and-forget and always reports success to iroh's QUIC stack, even when the underlying Veilid send actually fails — which live captures showed happening for several-to-tens-of-seconds after a freshly-attached embedded Veilid node dials out (its own network attach still settling, or the peer's route not yet resolvable via the DHT). The one-shot path-open ping from `VEILID-013` gets silently lost in that window, and QUIC abandons the path with nothing to ever retry it — a gap two earlier fix attempts missed (the abandon signal turned out to arrive as the *peer's* own timeout, not this node's; and the abandoned path's address is only recoverable if tracked separately, since a path that never validates never reaches the map that normally records it). Fixed by tracking a not-yet-validated path's address at open time and re-queuing it into the exact same open+ping retry machinery `VEILID-013` already built whenever it gets abandoned before validating (`VEILID-016`, `vendor/iroh-1.0.3/PATCH.md` Patch 5). Both `tests/veilid-smoke.sh` and the diagnostic loop's settle window were also raised from 120s to 240s: worst-case Veilid path validation can legitimately take 50+ seconds, and a retry needs a similar window of its own, so the original window sometimes ended mid-retry rather than validated or exhausted. Measured honestly, not just declared fixed: a larger, unattended batch at 240s (8 attempts) passed 6 of 8 (75%) — a real improvement over the pre-fix baseline, not a full elimination. The 2 remaining failures split into two different shapes: one matched this fix's own target exactly (the retry fired 4 times, each failing the same way — reads as genuinely sustained Veilid route-resolution unavailability to that peer, not a bug); the other is a distinct, not-yet-explained case where a path reached noq's own validated `Established` state and then still vanished with no abandon event logged at all — left as an open follow-up, possibly related to `register_and_configure_path` (the async-validation code path) never applying VEILID-014's idle-timeout override the way the synchronous open path does, unconfirmed.
...
- **`tetron status` no longer keeps showing a member who has left** (`STATUS-CACHE-001`). The status cache was invalidated only on local commands (the IPC denylist, and the embedder call above), but the cached data also holds `member_count` and each peer's live connection — and those change from background mesh tasks too: the coordinator pruning a member after its `tetron leave`, any node applying a fresh signed roster after a `MemberSync`, a peer disconnecting. After such an event the coordinator kept reporting the departed member — connection still shown as live — until the cache aged out (up to `status-cache.interval`, default 12s). Those three sites now drop the cache immediately, via a clone of the cache handle carried on the per-network context. No wire-format change.
...
- **The daemon no longer re-reads and re-parses its whole config tree on every status request** (`CONFIG-CACHE-001`). `config::load()` walked the config directory from scratch on each call, and `tetron status` called it twice per request — once directly, once per network. With `tetron-systray` polling every 8 seconds unconditionally, that worked out to roughly 1,800 file reads and 1,800 TOML parses per hour on any machine with a tray icon, all to rebuild a value that had not changed. The parsed config is now cached and validated against the files it came from (`settings.toml`'s size and mtime, the `networks/` directory mtime, and each network file's size and mtime), so a cached value is only used while the tree is genuinely unchanged. Deliberately not a timed cache: `tetron config set`, `join` and `leave` write the config from a *different process* than the running daemon, so there is no staleness window at all — a write from anywhere is picked up on the very next read.
...
- **A third, slower backoff tier for peers that have been unreachable a long time** (`CONVERGE-013`): the existing cold-peer cadence (`CONVERGE-011`, retries every 10 minutes once a peer has failed 10 consecutive dials) now escalates a second time — after roughly a day of continuous cold retrying (154 failed attempts by default), the cadence drops to once every 24 hours via new `tetron config set reconnect-frozen.threshold` / `reconnect-frozen.backoff` keys. This is a floor, not a stop: a peer that has been gone for months still gets redialed, at negligible cost, rather than the daemon giving up on it entirely (which would risk two mutually-unreachable-but-still-running peers never reconnecting if neither ever restarts). A peer coming back online is unaffected either way — it dials in from its own side immediately, regardless of how infrequently the far side was still trying.
...
- **Large UDP and ICMP packets are no longer silently corrupted** (`FRAG-004`): anything bigger than the tunnel's 1280-byte MTU is split by the host kernel before tetron ever sees it, so tetron receives the pieces, not the whole packet. When the peer connection's datagram ceiling was below 1280 as well (any network whose own MTU is modest, and briefly at the start of every connection), tetron had to split those pieces again — and rebuilt each one as though it were the start of a fresh packet, losing track of where it belonged and wrongly marking the packet as finished. The receiving machine then either dropped the whole thing or reassembled scrambled bytes, with no drop recorded and nothing in the logs. In practice this broke `ping -s 2000`, large DNS-over-UDP answers, and `iperf3 -u` at its default payload size, while bulk TCP was unaffected (TCP sizes its own segments to the tunnel MTU, so the kernel never pre-splits them) — which is why earlier fragmentation testing with `scp` and SSH passed while this was broken.
- **A peer can no longer crash a debug build of another node's daemon through a hostile transport parameter** (`FRAG-005`): the size ceiling tetron fragments against is partly the remote peer's to declare, and an absurdly small declared value ran the fragmenter's payload arithmetic past zero. Release builds wrap rather than panic, so the effect there was a single mis-sized fragment the connection then refused to send; a debug build panicked, and a daemon panic is deliberately fatal. Both cases are now refused up front. Reaching this at all requires already being an admitted member of the network. A matching bound was added to the IPv6 fragment envelope, which is unreachable in practice but was relying on an unchecked cast.
- **A coordinator now actually redials a member it can't reach, instead of trying exactly once and giving up forever** (`CONVERGE-012`): a coordinator's own dial mechanism only ever attempted each roster member once, at daemon/network restore — a member it couldn't immediately reach at that moment was never retried again for the rest of that process's life, and a coordinator's own mid-session disconnects only ever triggered cleanup (removing the dead route), never a reconnect attempt. This was especially costly for a promoted co-coordinator: restarting after promotion (a deploy, a crash, a routine restart) silently and permanently deleted its ability to redial any member it couldn't immediately reach at that restart, with no log line or status indicator surfacing it. A coordinator now spawns the same roster-aware, backoff-escalating retry task a member already gets (`CONVERGE-010`/`011`/`013`), so an unreachable member gets redialed identically regardless of which side is the coordinator.

- **Kicking or removing a peer now actually stops the daemon from redialing it** (`CONVERGE-010`): once a peer went offline, its reconnect retry loop checked nothing but its own backoff timer — so removing that peer from the network (`tetron kick`, or the peer departing while unreachable) never reached the loop, which kept dialing the gone peer every 30 seconds for as long as the daemon ran. Worse, if the removed peer later came back online, the stale loop would reconnect it into the data plane despite the roster saying it was gone. The retry loop now re-checks the network roster before every dial attempt and stops for good once the peer is no longer a member. Peers still in the roster are retried exactly as before — an unreachable but still-listed member keeps being redialed, which is the loop's job.
- **IP fragmentation now respects the same send-buffer backpressure as ordinary datagrams** (`FRAG-003`): the standard (unfragmented) send path has always dropped a new packet rather than send it when the peer's QUIC datagram send buffer is full, but the IPv4/IPv6 fragmentation paths had no equivalent check — under congestion, fragments were sent unconditionally regardless of buffer pressure. A packet is now sent whole or not at all: if the connection cannot take the entire fragment set, none of it goes out, rather than committing the first fragments and abandoning the rest (which left the receiving machine holding pieces of a packet that could never be completed, until its reassembly timeout expired). One drop is recorded for the packet, not one per fragment. Fragmentation's own behavior — which packets get fragmented, and how — is unchanged; this only affects what happens when the connection is already under backpressure.
- **Two more sources of journal log spam quieted** (`LOG-005`): a vendored dependency (`noq-proto`) was logging a `warn!` on every idle-timer tick for any connection without multipath negotiated — the common case, not an anomaly, and unrelated to connection health — now demoted to `debug!`. A persistently unreachable peer's "reconnecting in ... secs=N" line also re-logged at `info` on every backoff iteration (steady state: every 30s); it is now debounced the same way `PATH-DIAG-006` already debounces flapping-path logging, via new `tetron config set reconnect-log.threshold` / `reconnect-log.window` keys (defaults 3 attempts / 300s). Neither change affects reconnect behavior or multipath negotiation — logging only.
...
- **Path-event log noise reduced for any multi-homed peer** (`PATH-DIAG-005`/`006`/`007`): two independent subscribers were logging every path opened/closed/selected transition twice, and every node was unknowingly advertising its own tetron overlay IP as a raw NAT-traversal candidate to its peers (an upstream `netwatch` interface-enumeration gap — a `tun` device isn't excluded from local-address discovery), producing pointless open/close churn on every connection. Deduplicated to one subscriber; self-referential candidates are now filtered out entirely (`trace` only); a peer whose selected path genuinely oscillates between two real, similar-quality paths (any user with e.g. both wifi and ethernet up) now has its repeated `info`-level logging debounced per a new configurable window (`tetron config set path-flap.threshold` / `path-flap.window`, defaults 3 flips / 60s).
- **Connections no longer disconnect and reconnect every ~10 seconds while idle** (`CONN-STABILITY-001`, reverts `HARDEN-007`): a connection relying on relay (no direct path available) between two already-admitted, trusted mesh peers cycled disconnect/reconnect continuously while otherwise idle. Root cause: the relay protocol's own ping/pong keepalive runs on a ~15s cadence, but the QUIC idle timeout had been tightened to 10s — every relay-only connection died before that keepalive could ever fire even once. Reverted to iroh/quinn's own 30s default, which gives the relay's heartbeat enough room to engage and then sustain the connection indefinitely (confirmed: 0 reconnects over a 250s idle hold, vs. 14 in 180s at 10s). A genuinely idle direct connection was never affected by this.

### Changed

- **Reconnect attempts against a long-unreachable peer now slow down instead of redialing every 30 seconds forever** (`CONVERGE-011`): after 10 consecutive failed dials (roughly 4.5 minutes), the retry cadence for that peer escalates gradually from the usual 30-second cap to 10 minutes. A roster member that is merely offline for a while loses nothing — when it comes back, it dials in from its own side immediately, so reconnection speed is unaffected; the slower cadence only bounds the wasted outbound connection attempts against machines that are gone for good but still listed in the roster. The retry loop also now stands down when the peer has already reconnected inbound in the meantime, instead of redundantly replacing that healthy connection on its next attempt. Both knobs are configurable: `tetron config set reconnect-cold.threshold <attempts>` / `reconnect-cold.backoff <seconds>`.
...
- **QUIC idle timeout tightened from 30s to 10s** (`HARDEN-007`): every connection this node is party to now closes after 10s of inactivity from our side (the true bound is the minimum of both peers' settings), rather than iroh/quinn's 30s default — faster cleanup of scanner/probe connections that complete a handshake then go silent. Legitimate mesh connections are unaffected; they exchange traffic well within either bound.
...
- **`contrib/install-tetron-suite.sh` component selection replaced with an addon-aware picker** (`ADDONS-SUITE-002`): positional component names (`core`/`webui`/`systray`/`backup`) are gone, replaced by `--core-only`/`--install-webui`/`--install-systray`/`--install-backup`/`--install-all`. With none of those flags, the script now picks for itself: run at a real terminal (the README's `curl | bash` included — prompts read from `/dev/tty`, not stdin, so they survive being piped, the same trick rustup/Homebrew's installers use) it detects a display (`$DISPLAY`/`$WAYLAND_DISPLAY`) and offers `core, webui, systray` as the one-keystroke default (`core` alone if headless — `tetron-systray` cannot run without a display at all), with a picker for anyone who wants to deviate; with no controlling terminal at all (cron, CI, a container run without `-it`) it defaults to core-only rather than guessing. `backup` is never in the default set at any tier — opt-in only. Also fixes a latent bug found along the way: `confirm_core` (now `confirm_sudo_install`) required `--yes-core` even for a *fresh* install with no TTY, so the documented one-liner never actually installed `core` when run non-interactively — a fresh install now proceeds without `--yes-core` (nothing running yet, nothing to disrupt), while an upgrade of an already-installed `core`/`backup` still requires it or an interactive confirmation.

### Performance

- **Fragmenting a packet no longer allocates memory for each fragment** (`FRAG-006`): ordinary packets have been passed from the TUN to the network without any per-packet allocation since the zero-copy hand-off landed, but the oversized-packet path still built each fragment in a freshly allocated buffer and then copied it again on the way out — about four allocations and two copies per fragmented packet. Fragments now come out of the same kind of reused pool the ordinary path uses. Measured at 184ns down to 123ns per fragmented 1280-byte packet (33% faster; `cargo bench --bench forward -- fragment`). This falls on nodes whose connections sit below the tunnel MTU — a modest underlay MTU, or the opening round trips of any connection — where fragmentation is routine rather than exceptional.

### Internal

- **`config.rs` modularized (`MODULARIZE-003/004`)**: split into `config/schema.rs` (on-disk types), `config/overrides.rs` (relay/discovery resolution + `config set`/`config get` dispatch), and `config/storage.rs` (filesystem persistence); `config.rs` itself is now a re-export shim. Test module split to match, landed in the same commit (unlike `membership.rs`'s two-commit precedent, several storage functions are deliberately private test-seam variants invisible from the shim even via glob re-export, so there is no working intermediate state). No behavior change: every `crate::config::…` path still compiles via re-export, no wire/TOML format changed, and the same 280 lib tests pass, only relocated. Also fixes `config_dir()`'s stale macOS doc comment (claimed `~/.config/tetron`; the code has always actually used `~/Library/Application Support/tetron`) and three `reconcile.py` checks (`CON-001`/`CON-M04`/`CON-M03`) that hardcoded `src/config.rs`'s old path.
- **Invite-code codec moved into `tetron-proto` (`INVITE-010`)**: `encode_invite_code`/`decode_invite_code`/`is_bare_room_id` now live in `tetron-proto::invite`, so GUI frontends can call the real implementation instead of hand-copying it — `tetron-webui` currently hand-copies `decode_invite` with its own comment noting the function "lives in a binary crate not meant to be imported". `src/invite.rs` keeps a `pub use` re-export, so every `crate::invite::…` path (including `tetron-mobile`, which embeds the full `tetron` crate directly) keeps compiling and calling unchanged. `ReusableKey`/`InviteEntry` (the record types) stay in `tetron`'s own `src/invite.rs`. No wire-format change, no ALPN bump — purely where the source lives.
...
- **Default file-log verbosity caused real CPU overhead and unreadable logs** (`LOG-003`): the compiled default emitted a `debug`-level log line for every single TUN packet read/routed/dropped, which one real deployment measured at ~20% continuous CPU from formatting/writing overhead and thread lock contention alone. Default file-log level is now `info`; the five per-packet forwarding events are `trace`, not `debug`, so turning verbosity back up (`tetron config set log-level debug`) is now actually useful instead of reproducing the same flood.

### Changed

- **`RUST_LOG` no longer affects console output, only the file log** (`LOG-003`): previously, setting `RUST_LOG` silently overrode the console layer's own `info` filter too, so a manual override meant to inspect the file log also flooded the console/journal. Console output is now unconditionally `info` regardless of `RUST_LOG` or the new `log-level` config key.
...
- **`tetron config set log-level <trace|debug|info|warn|error>`** (`LOG-003`): persisted override for the daemon's file-log verbosity, applied on restart — no systemd drop-in needed to turn heavy logging on/off. `RUST_LOG` still wins over this if set.
...
- **musl release binaries (PORTABILITY-001)**: `tetron-linux-x86_64-musl` and `tetron-linux-aarch64-musl` join the existing glibc-linked releases -- a fully static binary with no dynamic libc dependency at all, so it runs unchanged across host glibc versions and on musl-only distros (Alpine). Also fixes a real crash found while verifying this: `noq-udp` (a transitive iroh dependency) asserted an alignment guarantee for control-message handling that doesn't hold under musl (`cmsghdr`'s layout differs from glibc's), which coredumped the daemon on startup -- patched locally (`vendor/noq-udp-1.1.0/`, not yet reported upstream) and re-verified end to end (install, create, status) on a real Rocky Linux 9 VM.
...
- **Overlay self-capture: a node's own kernel could swallow iroh's own dial attempts to a peer's overlay IP (SELFCAPTURE-ROUTE-001)**: every tetron node's TUN device installs an OS route for its overlay subnet, which iroh doesn't know is virtual -- it could offer a peer's own overlay IP as a direct-dial candidate, and any peer sharing that same subnet route locally swallowed the resulting packet into its own kernel/tetron forwarder instead of ever reaching the real remote host, causing relay-fallback flapping and higher latency than necessary on every peer pair. Fixed by routing iroh's own outbound traffic around every overlay subnet route (Linux: an `ip rule`/shadow routing table; macOS: a `pf` sub-anchor), applied automatically at daemon startup. On by default; disable with `tetron config set selfcapture-mitigation off`.
...
- **The physical-LAN subnet-overlap guard (SUBNET-012) silently did nothing on macOS, and only ran once at daemon boot (SUBNET-COLLISION-002)**: `check_subnet_overlap` unconditionally shelled out to the Linux-only `ip` binary with no macOS fallback, so its "fail-open if the tool is missing" design meant the check was a complete no-op on macOS since it shipped. It also only validated the node-wide default subnet at daemon startup, not a specific network's actually-resolved subnet at `create`/`join` time. Fixed: a macOS branch (parsing `ifconfig`) closes the platform gap, and the same overlap check now also runs at `create`/`join` time against the resolved subnet, gated by the same `--force` flag as SUBNET-COLLISION-001.
...
- **`tetron kick` could not remove a coordinator, ever, even the sole survivor's own dead co-coordinator (KICK-COORDINATOR-001)**: `kick` refused unconditionally against any coordinator target, with no override. Since a coordinator's roster entry never expires, a machine that permanently died or was reinstalled left a stale `is_coordinator` entry nothing could remove -- which could silently defeat `tetron leave`'s own stranding-safety check for a genuinely-solo surviving coordinator (they'd see 2 "coordinators" on paper and leave with no warning, stranding the network anyway) and could permanently deadlock `nuke`'s consensus gate the same way. Any coordinator may now kick any other coordinator, unilaterally, no consensus required -- the same unilateral trust `kick` already had for ordinary members. This is roster removal, not key revocation: a kicked coordinator whose machine is not actually gone still holds a working copy of the network's secret key and could mint themselves a fresh invite to rejoin; see `docs/HOWTO.md`'s "Kick a member" section for the full caveat. Self-kick is still refused (`leave`/`nuke` are the tools for that).
...
- **`tetron status --json`'s per-network `name` field gains a `network` twin (STATUS-NETWORK-FIELD-001)**: `name` reads stale next to `subnet`/`network_key`/etc. Both fields are populated identically for now -- `name` is not removed yet, since `tetron-webui`/`tetron-systray` both read it as a direct Rust field, not just a JSON key. New code (including tetron's own CLI) should read `network`; `name` stays until every consumer across the fleet has migrated off it.
- **Control-plane rate limiting hardened (HARDEN-002/004/005)**: a 2026-07-23 security review found the per-connection control-message gate (`ControlGate`) let an already-admitted peer burst 20 messages instantly (`CAPACITY=20`/`REFILL_PER_SEC=2`/`STRIKE_LIMIT=100`), each potentially triggering a pkarr resolve and blob fetch -- tightened to `CAPACITY=5`/`REFILL_PER_SEC=1`/`STRIKE_LIMIT=20`. A new daemon-wide `GlobalRateLimiter` (`GLOBAL_CAPACITY=10`/`GLOBAL_REFILL_PER_SEC=3`/`GLOBAL_STRIKE_LIMIT=50`) is consulted alongside every connection's own gate, bounding the aggregate control-plane workload across all connections rather than just one. Both sets of constants are now configurable: `tetron config set ratelimit.<capacity|refill-per-sec|strike-limit|global-capacity|global-refill-per-sec|global-strike-limit> <value>` (empty resets to the compiled default), applying on `sudo tetron restart` like every other `tetron config` key.
- **MTU/fragmentation diagnostics surfaced in `tetron status` (MTU-DIAG-001)**: directly motivated by the F-04 regression below -- diagnosing it required manually grepping a live peer's raw log file, since nothing about fragmentation activity or drop reasons was ever visible through the CLI. `tetron status --json` now reports each peer's live QUIC datagram-size ceiling (`connection.max_datagram_size`), a full per-reason drop breakdown, and `fragmented_ipv4`/`fragmented_ipv6` counts (packets successfully split, once per packet). The plain-text view gains one new line -- `drops N total (reason count, ...)` -- shown only when something has actually dropped, so a healthy daemon's output is unchanged.
- **Five more hardcoded constants are now configurable (CONFIG-AUDIT-002)**: `tetron config set nuke-proposal-ttl <duration>` (default 24h), `listen-port <port>` (default 43737, daemon-wide), `poller-interval <seconds>` (default 60), `log-retention <days>` (default 7), and `invite-default-expiry <duration>` (default 7d, also now honored by the invite auto-minted at `tetron create`) -- same shape as the existing `relay`/`discovery-dns`/`subnet`/`ratelimit.*` keys (empty value resets, applies on `sudo tetron restart`).
- **Manual DHT/group-poller trigger: `tetron sync [--network <name>]` (SYNC-001)**: previously the only way to learn about a fresh blob change (a newly minted invite, an admin grant) was to wait for the poller's own interval. `tetron sync` wakes it immediately instead, scoped to one joined network or every one; a built-in 2s cooldown prevents a spammed trigger from forcing back-to-back DHT resolves. Open to any local user, same as `tetron status` -- it causes no local mutation.
...
- **`tetron leave` could not remove a network that had never successfully restored (LEAVE-STUCK-NETWORK-001)**: a network whose restore failed outright (DHT/blob unreachable, no config fallback either) never got a `self.networks` entry, and both of `leave`'s resolution paths only ever scanned that live map -- so a stuck network had no CLI cleanup path at all short of deleting `networks/<name>.toml` directly as root. `leave` now falls back to an exact match against the persisted config when the live-only resolver fails.
- **IPv4 fragmentation silently "healed" a corrupted header instead of rejecting it (F-04)**: `fragment_ipv4` forwarded fragments built from the original packet's header fields without ever checking the original header's own checksum was valid -- each fragment got a freshly computed, valid checksum regardless of whether the source was ever intact. Now verifies the stored checksum first and refuses to fragment on a mismatch, matching normal IP processing.
- **Oversized IPv6 mesh packets were silently dropped instead of fragmented (FRAG-002)**: `FRAG-001` fixed this for IPv4, but the IPv6 arm still dropped any packet that exceeded the QUIC connection's `max_datagram_size()` -- so a TCP connection routed over a peer's tetron IPv6 address (rather than its IPv4 one) could reproduce the exact same silent SSH-hang-at-KEX bug FRAG-001 fixed, unaffected by that earlier fix. IPv6 has no in-header fragmentation fields and, unlike IPv4, RFC 8200 reserves fragmenting to the packet's true originating host -- so this is implemented as a tetron-internal envelope (`packet::fragment_ipv6`) rather than a literal RFC 8200 Fragment extension header, reassembled peer-side by a new per-connection `packet::Ipv6Reassembler` in `spawn_peer_reader` before the packet ever reaches the TUN writer.
- **F-04's own checksum check regressed IPv4 fragmentation completely (found live 2026-07-24)**: a stray extra bitwise NOT in the verify comparison made it reject every packet unconditionally, valid or not, so no oversized IPv4 packet could ever fragment -- a full regression of the original FRAG-001 bug, hit live at the exact same MTU numbers as the original 2026-07-15 report. The same stray negation was also present on the fragment-write side since FRAG-001's original commit, and in every test helper computing a synthetic checksum, so the whole existing test suite was checking its own (equally inverted) convention rather than truth -- no unit test ever exercised "a valid header must be accepted," only "a corrupted one must be rejected." Fixed at both sites; added a test for the missing direction plus an independent ground-truth check against a known-good real-world header checksum. Re-verified live: a 20MB transfer over the same mesh/MTU condition now fragments and reassembles cleanly with zero drop warnings.
- **`tetron admin <net> list`/`tetron invite <net> list` were denied for a non-operator local user (AUTHZ-001)**: found while scoping `SYNC-001`'s own authorization level -- both are pure reads (`AdminList`'s own doc comment already said "Open read"), but `check_authorized`'s exemption had only ever named `Status`, so either command returned "permission denied" for an unprivileged caller despite `AGENTS.md` describing reads as open to any local user. `invite list`'s own independent per-network coordinator check (unaffected by this fix) still applies underneath.
