...
- **Experimental Veilid transport** (`VEILID-001`..`015`, requires building with `--features veilid`). `tetron create --veilid` / `tetron join --veilid` (mutually exclusive with `--tor`) start a real embedded Veilid node and register it as a custom transport on the shared endpoint; the join/reconnect handshake propagates each peer's Veilid identity through the signed roster, and the dial path resolves it into an address for iroh to route through the custom transport. Live-verified via `tetron-testsuite`, across several rounds of found-and-fixed bugs:
  ...
  - **Still open (`VEILID-007`):** the mechanism is now confirmed working end-to-end — but only under `SafetySelection::Unsafe` (direct Veilid send, no sender-anonymizing safety route). The intended default, `SafetySelection::Safe`, never once delivered a message across this entire investigation despite reporting local send success every time — safety-route allocation between two freshly-bootstrapped nodes appears unreliable or too slow within any settle window tested so far. Whether to ship with `Unsafe` (working, less private within the Veilid network) or keep pursuing `Safe` (the originally intended privacy properties, not yet working) is a deliberate product decision, not resolved here. Not yet usable for real Veilid-carried mesh traffic pending that decision.
