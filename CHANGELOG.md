...
- **`--tor` dial-path candidate injection** (`TOR-DIAL-001`): the Tor custom transport's own onion-address candidate was never actually offered to the mesh dial path, so `--tor` connected peers over Direct/Relay only and never carried real traffic over Tor, silently. `connect_to_peer_with_alpn` now injects it the same way it already does for Veilid's roster-carried identity. Also fixed a stale-path retry gap in the vendored `iroh` copy (a backup path abandoned due to a local network change was never retried), an unbounded SOCKS5 connect and missing publish-confirmation wait in the vendored `iroh-tor-transport` copy (Tor's own `HS_DESC UPLOADED` event now gates when the transport considers itself ready, instead of returning the instant the hidden service is created locally), an event-name validator bug in the vendored `torut` copy that was silently blocking that wait from ever subscribing, and (found via a real two-machine, two-network live test, not the VM testsuite) a v3 onion descriptor quorum bug: the publish wait declared success after just one HSDir confirmation instead of the ~8 Tor's own spec requires, so dialing could start before the descriptor had reached most of the directories a client would actually query. **Known limitation:** with all of the above fixed and live-confirmed (the specific `Host unreachable`/HSDir-lookup failure no longer occurs), a second, distinct failure remains after descriptor quorum is reached — Tor's own client log shows every one of the target's introduction points getting marked unusable, so the actual circuit-level connection to a freshly-created onion service still fails (now a 30s timeout rather than an instant rejection). Not yet root-caused — see `spec/core.py`'s `TorDialPathWiring` for the complete investigation.
