...
- **New addon: [`tetron-hosts`](https://github.com/ErikAllanKincaid/tetron-hosts)** syncs peer hostnames into `/etc/hosts` as `<hostname>.<network>` (e.g. `node1.testing-delete-me`) -- no DNS server, no port 53, reads only the existing `tetron status` IPC. Per-network opt-in, scheduled via a systemd timer / launchd periodic job rather than a persistent daemon (`tetron-hosts sync` also always works by hand). `install-tetron-suite.sh` now offers it as a fifth component (`--install-hosts`, `ADDONS-SUITE-003`) alongside core/webui/systray/backup, opt-in only in the interactive picker (registers a root-level system service, unlike webui/systray's per-user ones).
