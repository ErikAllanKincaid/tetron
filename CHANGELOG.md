...
- **IP fragmentation now respects the same send-buffer backpressure as ordinary datagrams** (`FRAG-003`): the standard (unfragmented) send path has always dropped a new packet rather than send it when the peer's QUIC datagram send buffer is full, but the IPv4/IPv6 fragmentation paths had no equivalent check — under congestion, fragments were sent unconditionally regardless of buffer pressure. Both fragmentation paths now check available send-buffer space before each fragment, stopping the remainder of a fragment set (recording one drop, not one per fragment) rather than sending fragments a receiver could never reassemble into anything useful anyway. Fragmentation's own behavior — which packets get fragmented, and how — is unchanged; this only affects what happens when the connection is already under backpressure.
