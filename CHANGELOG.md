...
- **Experimental Veilid transport** (`VEILID-001`..`005`, requires building with `--features veilid`). `tetron create --veilid` / `tetron join --veilid` (mutually exclusive with `--tor`) start a real embedded Veilid node and register it as a custom transport on the shared endpoint; the join/reconnect handshake propagates each peer's Veilid identity through the signed roster, and the dial path resolves it into an address for iroh to route through the custom transport. Live-verified via `tetron-testsuite`: fixed a severe bug where a `--veilid` network could crash-loop the *entire* daemon on every restart (identity/attachment resolution blocked the whole startup sequence, not just Veilid — confirmed over 9+ restart cycles before the fix); both now resolve fully in the background instead (`VEILID-005`). What's still outstanding, found in that same live run: a `--veilid` network's roster rarely gets a real Veilid identity within one daemon session (the daemon only takes a short, bounded snapshot at boot, and real resolution has taken several minutes), so `conn_type` was still observed as `Direct` rather than `Veilid` between two peers — the mechanism carries real traffic once addressed, but nothing yet keeps that snapshot current after boot. A persisted Veilid identity across daemon restarts is also still outstanding (currently regenerated fresh each start). Tracking further work in `spec/core.py`.
