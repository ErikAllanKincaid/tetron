...
- **Kicking or removing a peer now actually stops the daemon from redialing it** (`CONVERGE-010`): once a peer went offline, its reconnect retry loop checked nothing but its own backoff timer — so removing that peer from the network (`tetron kick`, or the peer departing while unreachable) never reached the loop, which kept dialing the gone peer every 30 seconds for as long as the daemon ran. Worse, if the removed peer later came back online, the stale loop would reconnect it into the data plane despite the roster saying it was gone. The retry loop now re-checks the network roster before every dial attempt and stops for good once the peer is no longer a member. Peers still in the roster are retried exactly as before — an unreachable but still-listed member keeps being redialed, which is the loop's job. the standard (unfragmented) send path has always dropped a new packet rather than send it when the peer's QUIC datagram send buffer is full, but the IPv4/IPv6 fragmentation paths had no equivalent check — under congestion, fragments were sent unconditionally regardless of buffer pressure. Both fragmentation paths now check available send-buffer space before each fragment, stopping the remainder of a fragment set (recording one drop, not one per fragment) rather than sending fragments a receiver could never reassemble into anything useful anyway. Fragmentation's own behavior — which packets get fragmented, and how — is unchanged; this only affects what happens when the connection is already under backpressure.
