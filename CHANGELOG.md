...
- **`--tor` dial-path candidate injection** (`TOR-DIAL-001`): the Tor custom transport's own onion-address candidate was never actually offered to the mesh dial path, so `--tor` connected peers over Direct/Relay only and never carried real traffic over Tor, silently. `connect_to_peer_with_alpn` now injects it the same way it already does for Veilid's roster-carried identity. Also fixed: a stale-path retry gap in the vendored `iroh` copy (a backup path abandoned due to a local network change was never retried); an unbounded SOCKS5 connect and missing publish-confirmation wait in the vendored `iroh-tor-transport` copy; an event-name validator bug in the vendored `torut` copy blocking that wait from ever subscribing; a v3 onion descriptor quorum bug (the publish wait declared success after just one HSDir confirmation instead of the ~8 Tor's own spec requires, live-confirmed to eliminate the `Host unreachable`/"No more HSDir available to query" failure it targeted); a real side-effect-in-a-retried-path bug where a fixed-listen-port collision caused the Tor onion service to be built twice, tearing the first one's introduction circuits down mid-flight (the Tor transport is now built exactly once, reused across any port-bind retry); a concurrency bug where every outbound packet during connection setup independently raced its own fresh SOCKS5 connect instead of sharing one in-flight attempt (confirmed live: cut connection churn from ~146/5min to single digits); and a too-tight 30s connect timeout that was cutting off Tor's own normal multi-cycle rendezvous establishment before it could complete, with this codebase's own abandoned SOCKS5 socket then read by Tor's client as every introduction point having failed (raised to 90s). **Status:** all of the above are individually real, fixed, and (except the last two, pending re-verification after a test-hardware interruption) live-confirmed; end-to-end delivery has not yet been confirmed working in full — see `spec/core.py`'s `TorDialPathWiring` for the complete investigation.
