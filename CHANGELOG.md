...
- **Experimental Veilid transport** (`VEILID-001`..`013`, requires building with `--features veilid`). `tetron create --veilid` / `tetron join --veilid` (mutually exclusive with `--tor`) start a real embedded Veilid node and register it as a custom transport on the shared endpoint; the join/reconnect handshake propagates each peer's Veilid identity through the signed roster, and the dial path resolves it into an address for iroh to route through the custom transport. Live-verified via `tetron-testsuite`, across several rounds of found-and-fixed bugs:
  ...
  - **The actual final defect**, found only after a live test built specifically to remove every remaining confound (a coordinator-only restart whose restore-time dial already has the correct candidate, no racing or propagation delay possible) still showed zero activity from a single, clean, correctly-addressed dial: the ping-after-open fix from `VEILID-007`'s own earlier vendored-iroh patch lived in the wrong place. It pinged a newly-opened path from inside the function whose *own* open attempt almost always fails with `RemoteCidsExhausted` (the connection is milliseconds old) — the real, successful open always happens moments later via a different retry code path that the ping call never covered. The path reliably opened, real `PathId` and all; nothing ever pinged it, for this entire investigation. Fixed by moving the ping into the one place a path ID is ever newly assigned, covering every caller uniformly (`VEILID-013`, `vendor/iroh-1.0.3/PATCH.md` Patch 3).
  - **Still open (`VEILID-007`):** fixes applied through `VEILID-013` account for every symptom observed across this entire investigation; a live end-to-end re-test against all of them together is next, before marking this resolved. Not yet usable for real Veilid-carried mesh traffic.
