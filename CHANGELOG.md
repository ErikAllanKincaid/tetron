...
- **Experimental Veilid transport** (`VEILID-001`..`012`, requires building with `--features veilid`). `tetron create --veilid` / `tetron join --veilid` (mutually exclusive with `--tor`) start a real embedded Veilid node and register it as a custom transport on the shared endpoint; the join/reconnect handshake propagates each peer's Veilid identity through the signed roster, and the dial path resolves it into an address for iroh to route through the custom transport. Live-verified via `tetron-testsuite`, across several rounds of found-and-fixed bugs:
  ...
  - **The final piece, confirmed live rather than inferred:** even with every fix above in place, a live re-test still showed zero Veilid path activity. Temporarily promoting the vendored iroh's own trace logging for one diagnostic run (reverted before committing) showed the Veilid candidate genuinely opening as a real path — proving `VEILID-011` correct — but on a `noq::Connection` object that lost a race to a *different* connection this node's own code had independently started to the same peer within the same restart window (the member's reconnect loop, the coordinator's dial-retry, and the initial roster dial-out can all fire concurrently after a disconnect, and iroh creates a separate connection object per call rather than deduplicating them). The winning connection simply discarded the Veilid-carrying connection's paths along with it. This is exactly the connection-racing theory `VEILID-007`'s investigation suspected from the start, now confirmed directly. Closed with a small per-peer "dial already in flight" claim (`MeshCtx::dial_in_flight`/`DialInFlightGuard`) applied at all four of this node's own concurrent dial call sites, so only one is ever in flight to a given peer at a time — generic connection-establishment hygiene that benefits every transport, not just Veilid (`VEILID-012`).
  - **Still open (`VEILID-007`):** fixes applied through `VEILID-012` account for every symptom observed so far; a live end-to-end re-test against all of them together is next, before marking this resolved. Not yet usable for real Veilid-carried mesh traffic.
