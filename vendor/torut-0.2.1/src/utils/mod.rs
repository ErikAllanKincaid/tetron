...
/// `SETEVENTS` call
///
/// tetron-local patch (PATCH.md, TOR-DIAL-001 follow-up): the control-spec's
/// own event names are not all pure uppercase-ASCII -- `HS_DESC` and
/// `HS_DESC_CONTENT` both contain an underscore, which the original
/// `is_ascii_uppercase()`-only check rejected as `InvalidEventName`,
/// blocking any caller (like `iroh-tor-transport`, TOR-DIAL-001) from ever
/// subscribing to hidden-service-descriptor publish confirmations. `_` is
/// the only non-uppercase-letter character any real event name in the spec
/// actually uses, so it's allowed alongside uppercase ASCII rather than
/// switching to a full allow-list of known event name strings.
