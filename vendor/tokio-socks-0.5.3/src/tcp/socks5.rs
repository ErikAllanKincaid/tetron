    ...
    S: Stream<Item = Result<SocketAddr>> + Unpin,
{
    ...
    async fn receive_reply<T: AsyncSocket + Unpin>(&mut self, tcp: &mut T) -> Result<TargetAddr<'static>> {
        ...
        if let Some(err) = connect_reply_error(self.buf[1]) {
            return Err(err);
        }
...
/// tetron-local patch (PATCH.md, TOR-DIAL-001 follow-up): maps a SOCKS5
/// CONNECT reply's status byte to the error it represents, `None` for
/// success (0x00). Extracted from `receive_reply` into a pure function so
/// the byte-to-error mapping -- in particular, that a reply byte outside
/// the standard RFC 1928 0x00-0x08 range is preserved via
/// `Error::ExtendedError` rather than collapsed into the previous
/// catch-all's misleadingly-named `UnknownAuthMethod` -- is directly
/// unit-testable without needing to drive the full async handshake state
/// machine. See `Error::ExtendedError`'s own doc comment for why this
/// matters (Tor's own SOCKS5 extended error codes, 0xF0-0xF7).
fn connect_reply_error(byte: u8) -> Option<Error> {
    match byte {
        0x00 => None,
        0x01 => Some(Error::GeneralSocksServerFailure),
        0x02 => Some(Error::ConnectionNotAllowedByRuleset),
        0x03 => Some(Error::NetworkUnreachable),
        0x04 => Some(Error::HostUnreachable),
        0x05 => Some(Error::ConnectionRefused),
        0x06 => Some(Error::TtlExpired),
        0x07 => Some(Error::CommandNotSupported),
        0x08 => Some(Error::AddressTypeNotSupported),
        other => Some(Error::ExtendedError(other)),
    }
}

#[cfg(test)]
mod connect_reply_error_tests {
    use super::*;

    #[test]
    fn test_success_byte_is_none() {
        assert!(connect_reply_error(0x00).is_none());
    }

    #[test]
    fn test_standard_reply_codes_map_to_named_errors() {
        assert!(matches!(
            connect_reply_error(0x04),
            Some(Error::HostUnreachable)
        ));
        assert!(matches!(
            connect_reply_error(0x05),
            Some(Error::ConnectionRefused)
        ));
    }

    #[test]
    fn test_tor_extended_error_codes_preserve_the_raw_byte() {
        // Tor's own SOCKS5 extended error codes (socks-extensions.html):
        // 0xF0 descriptor not found, 0xF2 introduction failed, 0xF3
        // rendezvous failed, 0xF7 introduction timed out. Before this
        // patch, all four (and every other non-standard byte) collapsed
        // into the single, misleadingly-named `UnknownAuthMethod`.
        for byte in [0xF0u8, 0xF2, 0xF3, 0xF7] {
            match connect_reply_error(byte) {
                Some(Error::ExtendedError(b)) => assert_eq!(b, byte),
                other => panic!("expected ExtendedError({:#04x}), got {:?}", byte, other),
            }
        }
    }
}
