            ...
            None => {
                ...
                match ret {
                    ...
                    | Some(Err(PathError::MaxPathIdReached)) => {
                        ...
                        // PATH-DIAG-008 (tetron-local patch): `open_path_on_all_conns`
                        // already retries every queued address against every live
                        // connection on each tick, regardless of which connection
                        // queued it, so a duplicate entry here never carries
                        // distinct information -- it only multiplies the queue by
                        // the live connection count every 333ms for as long as the
                        // failure condition holds. See PATCH.md.
