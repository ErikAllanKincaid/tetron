                ...
                Timer::PerPath(path_id, timer) => {
                    match timer {
                        PathTimer::PathIdle => {
                            ...
                            {
                                // LOG-005 (tetron local patch): this fires on
                                // every PathIdle tick for any connection where
                                // multipath was never negotiated -- i.e. the
                                // common case, not an anomaly -- since
                                // close_path_inner's multipath-specific API is
                                // the wrong call for a plain single-path
                                // connection in the first place. warn! at this
                                // frequency (observed live: sustained, on both
                                // healthy Direct-connected peers and doomed
                                // reconnect attempts alike, unrelated to
                                // connection health) drowned out real signal
                                // in the journal. See tetron's own
                                // vendor/noq-proto-1.1.0/PATCH.md.
                                debug!(?err, "failed closing path");
                            }
                        }
