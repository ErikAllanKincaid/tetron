...
impl<'a, M: MsgHdr> Encoder<'a, M> {
    ...
    pub(crate) fn push<T: Copy>(&mut self, level: c_int, ty: c_int, value: T) {
        // PORTABILITY-001 local patch: same musl/glibc cmsghdr alignment
        // difference as `decode` below -- see that function's comment.
        // `ptr::write_unaligned` replaces the alignment-requiring
        // `ptr::write`, dropping the now-removed conservative assertion.
...
pub(crate) unsafe fn decode<T: Copy, C: CMsgHdr>(cmsg: &impl CMsgHdr) -> T {
    // PORTABILITY-001 local patch: musl's `cmsghdr` has a smaller alignment
    // than glibc's on the same arch (glibc's `cmsg_len` is `size_t`, 8 bytes
    // on x86_64; musl's is `socklen_t`, 4 bytes -- a real, documented ABI
    // difference, not a bug in this crate's general design), so the
    // conservative `align_of::<T>() <= align_of::<C>()` invariant this
    // function used to assert can be false under musl for perfectly valid
    // cmsg payloads (e.g. `libc::timespec`, 8-byte aligned). `ptr::read`
    // requires proper alignment and would be UB if the assertion were
    // simply removed; `ptr::read_unaligned` performs the equivalent copy
    // without that requirement, which is what's actually needed here --
    // the buffer's *content* is still exactly one `T`, only the compile-time
    // alignment guarantee differs from what `ptr::read` demands.
