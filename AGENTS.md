...
- Use `cargo -q` for all cargo commands; `tracing` for logging. `main::init_tracing` composes layers (console + file) with **decoupled filters** (LOG-003): console/CLI is unconditionally `info`, full stop — it never consults `RUST_LOG` or the config key, so it can't get noisy. Rolling daily files under `logdir::log_dir()` capture our crate at a level resolved from, in order, `RUST_LOG` (if set) → `tetron config set log-level <level>` → compiled default `info` (dependencies stay at `info` regardless). The five per-packet forwarding-path events (`forward.rs`'s TUN read/routing/send-failure lines) are `trace!`, not `debug!`, so `log-level debug` gives useful mesh/connection detail without per-packet flood; `log-level trace` is the full per-packet dump. A self-referential path-event candidate (a peer unknowingly advertising its own tetron overlay IP as a NAT-traversal candidate — an upstream `netwatch` gap, PATH-DIAG-007) is also `trace!`, since it can never be a real path at any verbosity above that; a genuinely oscillating `Selected` path is debounced per-peer (`path-flap.threshold`/`path-flap.window`, PATH-DIAG-006) rather than level-shifted. The vendored `noq-proto` dependency's `warn!` on every `PathIdle` timer tick for a non-multipath connection (the common case, not an anomaly) is demoted to `debug!` (`vendor/noq-proto-1.1.0/PATCH.md`, LOG-005), and `spawn_reconnect_loop`'s per-peer "reconnecting in" line against a persistently unreachable peer is debounced the same way as `path-flap` (`reconnect-log.threshold`/`reconnect-log.window`, LOG-005). A generic, callsite-keyed rate limiter (`log_ratelimit::RateLimitFilter`, LOG-006) additionally wraps `console_layer` itself, so any console line that starts repeating too fast at one call site — a dependency's own `warn!`/`info!` we don't control (e.g. iroh's `net_report` QAD-mapping-varies warning), or one with no domain-aware debounce written for it yet — is throttled automatically (`log-ratelimit.threshold`/`log-ratelimit.window`, default 5/60s) instead of needing a fresh incident-driven fix each time; it does not replace `path-flap`/`reconnect-log`, which stay as per-peer overrides underneath it. Returns a `LogGuard` that must stay alive for the process. `tetron config set/unset log-level` live-reloads a running daemon instead of requiring `sudo tetron restart` (LOG-004): the file layer's `EnvFilter` is wrapped in a `tracing_subscriber::reload::Layer`, whose `Handle` is registered process-globally in `log_reload` (mirroring `LogGuard`/the panic hook — one subscriber per process); the CLI writes `settings.toml` first, then best-effort notifies the already-running daemon over IPC (`IpcMessage::SetLogLevel`), falling back to the original restart-required message if no daemon is reachable.
...
- `transport::quic_transport_config()` sets no `max_idle_timeout` override — stays at iroh/quinn's own 30s default (`CONN-STABILITY-001`, reverting `HARDEN-007`'s 10s tightening). Do not re-tighten this: a relay-tunneled connection between admitted peers has no reliable activity of its own within a 10s window — the relay protocol's own ping/pong heartbeat runs on a ~15s+jitter cadence, so anything shorter than that guarantees the connection dies before the heartbeat ever gets a chance to fire even once. Live-verified regression coverage in `tetron-testsuite`'s `idle-connection-stability(-relay).sh`.
...
- Config under `config::config_dir()` (`/etc/tetron` on Linux; macOS uses `dirs::config_dir()` = `~/Library/Application Support/tetron`, `/var/root/...` for the root LaunchDaemon, the running user's own home for a manual per-user run): `secret_key`, `settings.toml`, `networks/<name>.toml` (one per network). Pre-migration installs auto-split the old `networks.toml`. On Linux the tree is `root:tetron`; secret-bearing files are `0600 root:root`. CLI commands that write identity directly need root on Linux since the tree is under `/etc`.
