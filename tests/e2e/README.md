...
|-----|-------|----------------|
| [`closed-net/`](closed-net) | 3 | Closed-net admission + lifecycle commands: live approval (`requests`/`accept`/`deny`), co-coordinator (`admin add`) gatekeeper resilience with a reusable key, `torpedo hostname` propagation, and `torpedo leave`/`nuke`. |
