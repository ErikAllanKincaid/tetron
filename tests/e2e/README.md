...
|-----|-------|----------------|
| [`closed-net/`](closed-net) | 3 | Closed-net admission + lifecycle commands: live approval (`requests`/`accept`/`deny`), co-coordinator (`admin add`) gatekeeper resilience (the co-coordinator admits by approval while the coordinator is offline), mesh-IP reachability, and `tetron leave`/`nuke`. |
| [`reliability/`](reliability) | 4 | Full-mesh packet-loss test: every pair probed both ways with `ping -c 1000 -i 0.01`, ICMP flood, and iperf3 UDP, over the tetron tunnel vs the direct public-IP baseline. Fails when tetron adds loss over the raw link. |
...
| `KEEP_STATE` | `0` | `1` skips the per-run tetron state wipe |
