...
/// The magic-DNS resolver IPv4 the OS is pointed at, as a string. Derived from
/// the node's operative overlay subnet ([`crate::dns::magic_dns_v4_node`]) so it
/// tracks a custom `--subnet` instead of being a hardcoded literal — the two
/// must stay equal (guarded by a unit test).
fn resolver_ip() -> String {
    crate::dns::magic_dns_v4_node().to_string()
}
...
pub fn restore_stale_backups() {
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        let backup = PathBuf::from(format!("/etc/resolver/{DNS_DOMAIN}.before-torpedo"));
        if backup.exists() {
...
mod macos {
    ...
    const SC_DNS_KEY: &str = "State:/Network/Service/torpedo/DNS";
    ...
    fn init_store() -> Result<&'static Mutex<SendSyncStore>> {
        ...
        let store = SCDynamicStoreBuilder::new("torpedo")
            .session_keys(true)
            .build()
    ...
    pub fn write_dns_config(search_domains: &[String], network_names: &[String]) -> Result<()> {
        ...
        let server_val = CFArray::from_CFTypes(&[CFString::new(&resolver_ip())]);
...
impl Resolvconf {
    fn iface_name(&self) -> &str {
        match self.variant {
            ResolvconfVariant::Debian => "tun-torpedo.inet",
            ResolvconfVariant::Openresolv => "tun-torpedo",
        }
    }
}
...
impl DnsConfigurator for Resolvconf {
    async fn apply(&self) -> Result<()> {
        ...
        use tokio::process::Command;
        let rip = resolver_ip();
        let config = format!("nameserver {rip}\nsearch {DNS_DOMAIN}\n");
...
fn render_direct_resolv_conf(search: &[String]) -> String {
    ...
    let rip = resolver_ip();
    s.push_str(&format!("nameserver {rip}\n"));
    if !search.is_empty() {
...
// Host-visible DNS artifacts carry the torpedo identity, not rayfish, so a
// genuine rayfish daemon on the same host never reads or clobbers our files
// (RENAME-006). The upstream relay/discovery preset URLs keep their rayfish
// names on purpose (they name upstream's servers, not a host artifact).
#[cfg(target_os = "linux")]
const BACKUP_SUFFIX: &str = ".before-torpedo";
#[cfg_attr(not(target_os = "linux"), allow(dead_code))]
const HEADER_COMMENT: &str = "# Added by torpedo - do not edit\n";

/// True iff `/etc/resolv.conf` contents are ours (carry the torpedo marker).
#[cfg_attr(not(target_os = "linux"), allow(dead_code))]
...
async fn reassert_resolv_conf(search: &[String]) -> Result<()> {
    ...
    if !resolv_conf_is_ours(&current) {
        tracing::warn!("/etc/resolv.conf was overwritten; re-asserting torpedo DNS");
...
#[cfg(target_os = "linux")]
const NM_DROPIN: &str = "/etc/NetworkManager/conf.d/torpedo-dns.conf";
...
mod tests {
    ...
    fn render_direct_resolv_conf_points_at_magic_ip() {
        ...
        assert!(out.starts_with("# Added by torpedo"));
        assert!(out.contains(&format!("nameserver {}", crate::dns::magic_dns_v4_node())));
