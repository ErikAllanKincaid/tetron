    ...
    match key {
        ...
        other => anyhow::bail!(
            "unknown config key: {other} (expected relay, discovery-dns, subnet, \
             ...
             log-ratelimit.<threshold|window>, or \
...
fn set_log_ratelimit_key(
    ...
    let sub = key.strip_prefix("log-ratelimit.").expect("checked by caller");
    if reset {
        match sub {
...
mod tests {
    ...
    fn config_set_get_log_ratelimit() {
        let mut cfg = AppConfig::default();
        assert_eq!(config_get(&cfg, Some("log-ratelimit.threshold")).unwrap()[0].1, "<default>");
        assert_eq!(config_get(&cfg, Some("log-ratelimit.window")).unwrap()[0].1, "<default>");

        config_set(&mut cfg, "log-ratelimit.threshold", "5", false).unwrap();
        assert_eq!(cfg.log_ratelimit.threshold, Some(5));
        assert_eq!(config_get(&cfg, Some("log-ratelimit.threshold")).unwrap()[0].1, "5");

        config_set(&mut cfg, "log-ratelimit.window", "120", false).unwrap();
        assert_eq!(cfg.log_ratelimit.window_secs, Some(120));
        assert_eq!(config_get(&cfg, Some("log-ratelimit.window")).unwrap()[0].1, "120");
