...
cargo -q run -- up                  # activate the VPN (unprivileged IPC if the daemon is already running)
cargo -q run -- down                # put the daemon on standby (TUN down, DNS reverted) — process keeps running
...
- `src/tun.rs` — async TUN device creation with both IPv4 (/10 netmask, via the `tun` crate) and IPv6 (`200::/7`) addresses, returns `(TunReader, TunWriter, String)` with tun name, split into TunReader/TunWriter for lock-free I/O; `configure_ipv6()` platform helper assigns the IPv6 address so the whole `200::/7` peer range routes into the TUN (mirroring the IPv4 /10 netmask): on Linux via pure-Rust netlink (`rtnetlink` crate — adds the address with a `/7` prefix so the kernel installs the connected route automatically, no subprocess), on macOS via `ifconfig` + an explicit `route add -inet6 -net 200::/7` (utun is point-to-point so the prefix alone does not create the range route)
...
**VPN activation / standby (up/down):** The daemon process and its iroh Endpoint, IPC socket, blob store, and metrics server are *always-on infrastructure* — separate from the *active VPN state* (TUN interface up + system DNS configured + networks connected). `DaemonState.active` (an `AtomicBool`) tracks which state the daemon is in. `DaemonState::activate()` brings the TUN link up (`tun::set_link_up`), configures system DNS (stored in `DaemonState.dns_configurator` so it can be reverted later), and reconnects every saved network; `deactivate()` tears down each network's runtime via `teardown_network_runtime()` (without removing it from config), reverts system DNS, and brings the TUN link down (`tun::set_link_down`). The daemon calls `activate()` once at startup (so a fresh boot behaves as before) and `deactivate()` on shutdown. `ray up` sends `IpcMessage::Up` → `activate()`; `ray down` sends `IpcMessage::Down` → `deactivate()`. Because the already-root daemon performs the privileged TUN/DNS work, `ray up`/`ray down` are **unprivileged** IPC calls — `ray up` only needs root when no daemon is reachable and it must install/start the system service. This mirrors Tailscale's `tailscaled` (always running) + `tailscale up/down` (client toggles) split.
...
- `rtnetlink` — (Linux only) pure-Rust async netlink client; used in `tun.rs` to assign the TUN's IPv6 address (with a `/7` prefix that auto-installs the `200::/7` route) without shelling out to `ip`
