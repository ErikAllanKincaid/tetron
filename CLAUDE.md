...
- `src/dns.rs` — Magic DNS resolver: UDP DNS server on `<my-ip>:53` (device's TUN IP), answers A queries for `*.pi` names from in-memory HostnameTable (network → hostname → IP), returns REFUSED for non-.pi queries; `spawn_dns_server(bind_ip, table, cancel)`, `HostnameTable` type, `new_hostname_table()`
- `src/dns_config.rs` — OS-level DNS configuration: `DnsConfigurator` trait with `apply()`/`revert()`, platform detection chain (macOS scoped resolver `/etc/resolver/pi`, Linux systemd-resolved/resolvconf/direct), resolver IP set to the device's TUN IP (not a fixed address), backup/restore of modified files (`.before-pitopi` suffix), crash recovery on daemon start
