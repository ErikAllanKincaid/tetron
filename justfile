...
deploy ip:
    ...
    ssh {{user}}@{{ip}} "getent group tetron >/dev/null || groupadd tetron && install -m 755 /tmp/{{binary}} /usr/local/bin/{{binary}} && (systemctl restart tetron 2>/dev/null || {{binary}} install)"
